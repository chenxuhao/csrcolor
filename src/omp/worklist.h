...
unsigned Worklist::pushRange(unsigned *copyfrom, unsigned nitems) {
	...
	unsigned offset = my_fetch_add<unsigned>(&end, nitems);
	if (offset >= lcap) {	// overflow.
		my_fetch_sub<unsigned>(&end, nitems);
		return 1;
	}
...
unsigned Worklist::popRange(unsigned *copyto, unsigned nitems) {
	...
	if (nitems) {
		if (start + nitems < lcap) {
			offset = my_fetch_add<unsigned>(&start, nitems);
		} else {
			offset = my_fetch_add<unsigned>(&start, start + nitems - lcap);
		}
	}
