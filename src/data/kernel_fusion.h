...
// Data-driven version with Kernel Fusion technique
#include <cub/cub.cuh>
#include "gbar.cuh"
...
__device__ void firstFit(int m, int *csrRowPtr, int *csrColInd, Worklist2 &inwl, Worklist2 &outwl, int *coloring) {
...
__device__ void conflictResolve(int m, int *csrRowPtr, int *csrColInd, Worklist2 &inwl, Worklist2 &outwl, int *coloring) {
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int total_inputs = (*inwl.dindex + gridDim.x * blockDim.x - 1)/(gridDim.x * blockDim.x);
	for (int id = tid; total_inputs > 0; id += blockDim.x * gridDim.x, total_inputs--) {
	//int perthread = (*inwl.dindex - 1) / (gridDim.x * blockDim.x) + 1;
...
__global__ void color_kernel(int m, int *csrRowPtr, int *csrColInd, Worklist2 inwl, Worklist2 outwl, int *coloring, GlobalBarrier gb) {
	Worklist2 *in;
	Worklist2 *out;
	Worklist2 *tmp;	
	in = &inwl; out = &outwl;
	while (*in->dindex > 0) {
		firstFit(m, csrRowPtr, csrColInd, *in, *out, coloring);
		gb.Sync();
		conflictResolve(m, csrRowPtr, csrColInd, *in, *out, coloring);
		gb.Sync();
		tmp = in;
		in = out;
		out = tmp;
...
void color(int m, int nnz, int *csrRowPtr, int *csrColInd, int *coloring, int num_SMs) {
	...
	printf("Graph coloring data-driven Kernel Fusion version\n");
	...
	//printf("nSM=%d, block_size=%d, max_blocks=%d\n", nSM, BLKSIZE, max_blocks);
	...
	for (int i = 0; i < ITERATIONS; i++) {
		...
		color_kernel<<<nSM * max_blocks, BLKSIZE>>>(m, d_csrRowPtr, d_csrColInd, inwl, outwl, d_coloring, gb);
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
		endtime = rtclock();
	...
	for (int i = 0; i < ITERATIONS; i++) {
		...
		printf("[%d %.2f %d] ", colors[i], runtime[i]);
	}
